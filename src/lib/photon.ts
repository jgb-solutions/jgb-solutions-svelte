...
export default function Photon({ src, width, quality = 80 }: {
  src: string
  width?: number
  quality?: number
}) {
  if (src[0] === "/") {
    ...
    src = "https://mp3pam.com" + src
  }

  const domainWithoutProtocol = src.replace(/(^\w+:|^)\/\//, "")

  const url = makePhotonUrl({ width, quality }, domainWithoutProtocol)
...
export const makePhotonUrl = (
  photon: PhotonInterface,
  domainWithoutProtocol: string,
) => {
  const photonHost =
